...
void Scene::init()
{
	...
	//Init enemics
		//Flowers
		flowerEnemies[0] = new EnemyFlower();
		flowerEnemies[0]->init(glm::vec2(83*16, 40*16), texProgram);

		flowerEnemies[1] = new EnemyFlower();
		flowerEnemies[1]->init(glm::vec2(99*16, 40*16), texProgram);

		flowerEnemies[2] = new EnemyFlower();
		flowerEnemies[2]->init(glm::vec2(106 * 16, 38 * 16), texProgram);

		flowerEnemies[3] = new EnemyFlower();
		flowerEnemies[3]->init(glm::vec2(113 * 16, 40 * 16), texProgram);

		flowerEnemies[4] = new EnemyFlower();
		flowerEnemies[4]->init(glm::vec2(124 * 16, 39 * 16), texProgram);
...
bool Scene::checkIfOnScreen(glm::ivec2 pos, glm::ivec2 size) {
	int margin = 80;
...
void Scene::updateScreen() {
	...
	for (EnemyFlower* f : flowerEnemies) {
...
bool Scene::collidesWithPlayer(glm::ivec2 posColliderEnemy, glm::ivec2 sizeColliderEnemy) {
...
void Scene::updateEnemy(int deltaTime, Enemy* enemy) {
...
void Scene::updateEnemiesOnScreen(int deltaTime) {
	//Para EnemyFlowers
	for (EnemyFlower* f : flowerEnemies) {
...
void Scene::renderEnemiesOnScreen() {
	for (EnemyFlower* f : flowerEnemies) {
